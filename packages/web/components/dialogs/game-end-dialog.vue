<template>
  <Dialog>
    ...
    <h2 class="text-center" v-if="winner">
      👑 {{ winner.username }} <span v-if="winnerPoints">| {{ winnerPoints }}</span>
    </h2>

    <Button variant="green" :disabled="room.creatorId !== room.player.sessionId"  @click="restart" small>
      {{ $t('t.restart') }}
    </Button>
    <Button variant="blue" @click="lobby"  :disabled="room.creatorId !== room.player.sessionId" small>{{ $t('t.toLobby') }}</Button>

    <small class="text-gray-400 block text-center mb-2 w-full"
          >{{ $t('t.onlyLeaderCan') }}</small>
