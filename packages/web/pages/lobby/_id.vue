  ...
  <div class="main-menu">
    <template v-if="room">
      <h1 class="text-sm mb-5 sm:text-xl sm:mb-3 text-left">
        <span class="text-gray-600">Raum: {{ room.roomId }}</span></h1>
      <Panel>
        <template #title>{{ $t('lobby.players') }}</template>
        <template #content>
          <LobbyPlayers :room="room"/>
          <Button v-if="isLeader" small v-else @click="start" :loading="true"
                  variant="blue">
            {{ $t('lobby.startGame') }}
          </Button>
      ...
      <GameSettingsView shade='blue' :room="room" v-if="!isLeader" :excluded="excludedSettings"/>
      <GameSettings shade='blue' v-model="settings" v-if="isLeader" :excluded="excludedSettings"/>
    </template>
  </div>
</template>
...
@Component({
  components: {Share, Panel, Button, LobbyPlayers, GameSettings, GameSettingsView}
})
export default class LobbyPage extends Vue {
  ...
  get isLeader() {
    return this.room && this.room.player && this.room.creatorId === this.room.player.sessionId;
  }
