<template>
  <div class="main-menu pt-10">
    <Panel v-if="games.length" class="flex flex-col" v-for="game of games" :key="game.roomId">
      <template #title>
        <div class="flex items-center w-full px-5 justify-between">
          {{ game.roomId }}
          <div class="flex items-center">
            🕹 {{ game.clients }}
          <Button class="hidden md:flex ml-3" variant="purple" x-small :to="`/lobby/${game.roomId}`">
          {{ $t('lobbies.join') }}
          </Button>
          </div>
...
export default class LobbiesPage extends Vue {
  ...
  async fetchGames(){
    ...
    for (const room of rooms){
      const previousGame = this.games.findIndex(game => game.roomId === room.roomId);
      if (previousGame >= 0){
