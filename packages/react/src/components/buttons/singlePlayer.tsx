...
export const SinglePlayerButton = ({title}) => {
    ...
    const [mapSet, setMapSet] = useState("world");
    ...
    const startGame = () => {
        dispatch(start(`game_${gameMode}`, { mapSet }))
    }
    ...
    return <FancyDialog visible={visible}>
        ...
        <FancyDialogContent>
            <HorizontalAlignment between={true}>
                <GameModeSlider onChange={setGameMode}/>
                <MapSetSlider onChange={setMapSet}/>
            </HorizontalAlignment>
            <WithPin flyIn={visible}>
                <SmallButton onClick={() => startGame()} variant={"green"} title={strings.startGame}/>
