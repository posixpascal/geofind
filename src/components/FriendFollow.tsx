...
export const FriendFollow: React.FC<FriendFollowProps> = ({onFollow}) => {
    ...
    return <div className={'flex flex-col gap-8'}>
        <header>
            <h3 className={'text-card-headline text-3xl font-black'}>{t('add.title')}</h3>
            <p className={'text-card-paragraph text-lg'}>{t('add.description')}</p>
        </header>

        <Input onChange={(ev) => setName(ev.target.value)} name="name" type="text" label={t('name')} placeholder={t('nameOrEmail').toString()}/>
        <div>
            <Input onChange={(ev) => setFriendCode(ev.target.value)} name="friendCode" type="text" label={t('friendCode')}
        ...
        <div>
            ...
            return <animated.div style={style} key={user!.id}>
