...
export const GameMapSelection = () => {
    // TODO: type
    const maps : Array<keyof typeof GameMap> = Object.keys(GameMap) as unknown as Array<keyof typeof GameMap>;
    const iconMapping: Record<keyof typeof GameMap, ReactNode> = {
        [GameMap.AFRICA]: <AfricaMap className={'h-[100px] w-[100px]'}/>,
        [GameMap.GLOBE]: <WorldMap className={'h-[100px] w-[100px]'}/>,
        [GameMap.WORLD]: <WorldMap className={'h-[100px] w-[100px]'}/>,
        [GameMap.SOUTH_AMERICA]: <SouthAmericaMap className={'h-[100px] w-[100px]'}/>,
        ...
        [GameMap.NORTH_AMERICA]: <NorthAmericaMap className={'h-[100px] w-[100px]'}/>,
    }

    return <div>
        <Swipes title={"Map"}>
            {maps.map((map) => (
                <div key={map} className={'flex flex-col mx-2 rounded-xl p-4 items-center justify-center'}>
                    {iconMapping[map]}
                    <div>
                        {map}
                    </div>
