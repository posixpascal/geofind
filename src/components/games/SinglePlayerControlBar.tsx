...
import { DistanceAnimation } from "../utils/DistanceAnimation";
...
export const SinglePlayerControlBar: React.FC<RoundPreparedDialogProps> = ({
  singlePlayer,
}) => {
  ...
  const marker = useSelector(() => markerState.get());
  ...
  return (
    <div
      className={
        "transform-gpu shadow-xl m-5 p-4 rounded-xl -translate-x-2/4 bg-card bg-opacity-60 backdrop-blur  left-[50%] absolute max-w-xl w-full top-0"
      }
    >
      <animated.div style={style}>
        <div
          className={
            "grid grid-cols-3 items-center justify-center text-center gap-2"
          }
        >
          <div className={"col-span-1 text-xl"}>Gesucht wird</div>
          <div className={"text-7xl"}>{singlePlayer.country?.flagEmoji}</div>
